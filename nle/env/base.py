...
SKIP_EXCEPTIONS = (
    b"eat", b"attack", b"direction?", b"pray", b"wield", b"wear", b"take off", b"drink", b"read",
    b"use or apply", b"drop", b"throw", b"put on", b"remove", b"zap",
)
SKIP_EXCEPTIONS_PATTERN = re.compile(b"|".join(SKIP_EXCEPTIONS))
...
class NLE(gym.Env):
    ...
    def __init__(
        ...
        num_action = len(self._actions)
        if item_actions:
            num_action += nethack.INV_SIZE[0]
        self._item_action_begin = len(self._actions)
        self._item_action_end = num_action
        ...
        must_keys = ["glyphs", "blstats", "message", "program_state", "internal"]
        if item_actions:
            must_keys.append("inv_letters")
        ...
        self._inv_letters_index = None
        if "inv_letters" in self._observation_keys:
    ...
    def step(self, action: int):
        ...
        if self._item_action_begin <= action < self._item_action_end:
            action_ord = last_observation[self._inv_letters_index][action - self._item_action_begin]
        else:
            action_ord = self._actions[action]
        observation, done = self.env.step(action_ord)
        ...
        while not done:
            ...
            if in_yn_function:  # Game asking for a single character.
                ...
                if exceptions:
                    ...
                    if self._allow_all_yn_questions:
                        break
                    if SKIP_EXCEPTIONS_PATTERN.search(msg) is not None:
                        break
