...
class ActorPool {
 public:
  ...
  void loop(int64_t loop_index, const std::string& address) {
    ...
    const TensorNest& initialLastAction = TensorNest(
        torch::full({1, 1}, 0, torch::dtype(torch::kInt64)));

    TensorNest env_outputs = ActorPool::step_pb_to_nest(&step_pb, initialLastAction);
    ...
    try {
      while (true) {
        ...
        for (int t = 1; t <= unroll_length_; ++t) {
          ...
          env_outputs = ActorPool::step_pb_to_nest(&step_pb, action);
  ...
  static TensorNest step_pb_to_nest(rpcenv::Step* step_pb, const TensorNest& lastAction) {
    ...
    TensorNest lastActionCopy = lastAction;

    return TensorNest(std::vector(
        {nest_pb_to_nest(step_pb->mutable_observation(), array_pb_to_nest),
         std::move(reward), std::move(done), std::move(lastActionCopy),
