...
assert subprocess.call(['git', 'submodule', 'update', '--init', '--recursive']) == 0
assert subprocess.call(['bash', 'scripts/install_grpc.sh']) == 0
...
if not PREFIX:
    prefix_path = Path(shutil.which('protoc')).resolve().parents[1]
    if prefix_path:
...
def main():
    ...
    setuptools.setup(
        ...
        install_requires=[
            'black>=20.8b1', 'flake8', 'gitpython', 'opencv-python',
            'pre-commit', 'gym[atari]', 'torch', 'torchvision', 'nest',
        ],
        dependency_links=[
            os.path.join(__file__, '../nest', 'nest-0.0.3-py3.7.egg'),
        ],
    )
